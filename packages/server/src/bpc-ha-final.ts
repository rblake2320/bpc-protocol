...
const GOVERNED_DML_TABLES=['public.ha_outbox_meta','public.ha_outbox_fence','public.ha_outbox_source_checkpoint','public.ha_outbox_receiver_checkpoint','public.ha_outbox_rows','public.ha_outbox_publisher_lease','public.ha_outbox_quarantine','public.ha_outbox_applied','bpc_ha.authority_stream','bpc_ha.cutover_head','bpc_ha.cutover_history','bpc_ha.epoch_witness','bpc_ha.mutation_ticket_key','bpc_ha.mutation_ticket_nonce','bpc_ha.promotion_attestation','bpc_ha.promotion_receipt','bpc_ha.redis_membership','bpc_ha.schema_meta','bpc_ha.source_activation','bpc_ha.source_lease','bpc_ha.source_lease_history'] as const;
...
BEGIN
  IF max_pending<1 OR (SELECT count(*) FROM jsonb_object_keys(ticket))<>15 OR ticket->>'action'<>'outbox-append' OR ticket->>'mac' !~ '^[0-9a-f]{64}$' OR ticket->>'opDigest' !~ '^[0-9a-f]{64}$' THEN RAISE EXCEPTION 'invalid outbox ticket'; END IF;
...
export interface DbMutationTicketRequest {readonly domain:'bpc-db-mutation/v1';readonly keyId:string;readonly nonce:string;readonly streamId:string;readonly epoch:string;readonly leaseId:string;readonly grantDigest:string;readonly txid:string;readonly expiresAtMs:string;readonly sourceEpoch:string;readonly sequence:string;readonly opDigest:string;readonly action:string;readonly payloadDigest:string;readonly policyDigest:string}
...
export function validateDbMutationPolicyContext(request:Readonly<DbMutationTicketRequest>,context:Readonly<DbMutationPolicyContext>,codec:PairPayloadCodec):void{
  ...
  const mutation=policy['mutation'] as Record<string,unknown>,payload=policy['payload'];if(!mutation||typeof mutation!=='object'||!Array.isArray(payload)||policy['action']!==request.action||policy['streamId']!==request.streamId||policy['sourceEpoch']!==request.sourceEpoch||policy['sequence']!==request.sequence)throw new ContractValidationError('mutation policy binding mismatch');bpcPairMutationSanitizer.assertSanitized(mutation);const clean=mutation as SanitizedMutation<BpcPairMutation>;
  ...
  const id0=String(payload[0]??'');let allowed=false;
  if(clean.kind==='bpc.pair.set.v1')allowed=request.action==='outbox-append'||request.action==='pair-upsert'&&id0===clean.pairId;
  else if(clean.kind==='bpc.pair.delete.v1')allowed=request.action==='outbox-append'||request.action==='pair-delete'&&id0===clean.pairId;
  else if(clean.kind==='bpc.pending.set.v1')allowed=request.action==='outbox-append'||request.action==='pending-upsert'&&id0===clean.token&&String(payload[2])===String(clean.requestedAt);
  else if(clean.kind==='bpc.pending.delete.v1')allowed=request.action==='outbox-append'||request.action==='pending-delete'&&id0===clean.token;
  else if(clean.kind==='bpc.pair.approve.v1')allowed=request.action==='outbox-append'||request.action==='pending-delete'&&id0===clean.token||request.action==='pair-insert'&&id0===clean.pairId;
  else if(clean.kind==='bpc.pair.rotate.v1')allowed=request.action==='outbox-append'||request.action==='pair-upsert'&&(id0===clean.oldPairId||id0===clean.newPairId);
  if(!allowed)throw new ContractValidationError('policy signer rejected mutation/action payload mapping');
  if(request.action==='pair-upsert'||request.action==='pair-insert'){
    const pairId=id0,kindValue=clean.kind as 'bpc.pair.set.v1'|'bpc.pair.approve.v1'|'bpc.pair.rotate.v1';let sealed:unknown;
    if(clean.kind==='bpc.pair.set.v1'||clean.kind==='bpc.pair.approve.v1')sealed=clean.sealed;
    else if(clean.kind==='bpc.pair.rotate.v1')sealed=pairId===clean.oldPairId?clean.expectedOld:clean.sealed;
    else throw new ContractValidationError('pair DML is not authorized by a sealed pair mutation');
    const opened=codec.openPair(sealed as never,{domain:'bpc-pair-payload',version:'1',streamId:request.streamId,kind:kindValue,pairId});
    const expected=[opened.id,opened.name,opened.scope,opened.mode,opened.secretHash,JSON.stringify(opened.pubJwk),opened.status,opened.created,opened.lastActive,opened.requests,opened.failedSigs,opened.cumulativeFailures??null,opened.firstFailureAt??null,opened.maxRequests??null,opened.kind??'legitimate',opened.canaryClass??null,opened.expiresAt??null];
    if(canonicalize(payload)!==canonicalize(expected))throw new ContractValidationError('sealed pair does not authorize the complete DML payload');
  }else if(request.action==='pending-upsert'){
    if(clean.kind!=='bpc.pending.set.v1')throw new ContractValidationError('pending DML is not authorized by a sealed registration');const opened=codec.openRegistration(clean.sealed,{domain:'bpc-pair-payload',version:'1',streamId:request.streamId,kind:'bpc.pending.set.v1',token:clean.token,requestedAt:clean.requestedAt});if(canonicalize(payload)!==canonicalize([clean.token,JSON.stringify(opened),clean.requestedAt]))throw new ContractValidationError('sealed registration does not authorize the complete DML payload');
  }
}
/** Separate policy boundary. A production implementation must authorize the
 * canonical outbox mutation -> action/payload mapping and current external
 * authority proof; it must not expose a general MAC oracle. */
...
export class PgSourceLeaseFence {
  ...
  private async issueTicket(exec:PgExecutor,header:OutboxRecordHeader,action:string,payloadJson:string,mutationJson:string):Promise<Record<string,string>>{
    ...
    const canonicalPolicy=String(clock?.policy_canonical),policyDigest=sha256(canonicalPolicy);const request=Object.freeze({domain:'bpc-db-mutation/v1' as const,keyId:this.mutationSigner.keyId,nonce,streamId:this.binding.streamId,epoch:String(this.binding.epoch),leaseId:this.binding.leaseId,grantDigest:this.binding.grantDigest,txid,expiresAtMs:String(expiresAtMs),sourceEpoch:header.sourceEpoch,sequence:String(header.sequence),opDigest:header.opDigest,action,payloadDigest,policyDigest}),context=Object.freeze({canonicalPolicy});const mac=await this.mutationSigner.signTicket(request,context);if(!/^[0-9a-f]{64}$/.test(mac))throw new ContractValidationError('mutation ticket signer returned invalid MAC');
...
export async function provisionBpcRuntimeMutationBoundary(db:PgTransactor,runtimeRole:string,keyId:string,secret:Uint8Array):Promise<void>{
  ...
  try{await db.transaction(async exec=>{const role=(await exec.query('SELECT rolsuper,rolbypassrls,rolinherit FROM pg_catalog.pg_roles WHERE rolname=$1',[runtimeRole])).rows[0];if(!role||role.rolsuper||role.rolbypassrls)throw new ContractValidationError('runtime role missing or holds bypass authority');
    ...
    await exec.query(`REVOKE INSERT,UPDATE,DELETE,TRUNCATE ON TABLE public.ha_outbox_meta,public.ha_outbox_fence,public.ha_outbox_source_checkpoint,public.ha_outbox_receiver_checkpoint,public.ha_outbox_rows,public.ha_outbox_publisher_lease,public.ha_outbox_quarantine,public.ha_outbox_applied FROM PUBLIC,${runtimeRole}`);
    await exec.query(`GRANT SELECT ON ALL TABLES IN SCHEMA bpc_ha TO ${runtimeRole}`);
    await exec.query(`REVOKE INSERT,UPDATE,DELETE,TRUNCATE ON ALL TABLES IN SCHEMA bpc_ha FROM PUBLIC,${runtimeRole}`);
    ...
    const authorityDml=(await exec.query(`SELECT 1 FROM pg_catalog.pg_roles r CROSS JOIN unnest($2::text[]) table_name CROSS JOIN unnest(ARRAY['INSERT','UPDATE','DELETE','TRUNCATE']) privilege WHERE pg_catalog.pg_has_role($1,r.oid,'MEMBER') AND pg_catalog.has_table_privilege(r.rolname,table_name,privilege) LIMIT 1`,[runtimeRole,GOVERNED_DML_TABLES])).rows[0];if(authorityDml)throw new ContractValidationError('runtime role membership retains governed authority/evidence DML');
...
async function assertBpcRuntimeMutationBoundaryExec(exec:PgExecutor,expectedRole?:string):Promise<string>{
  ...
  const authorityDml=(await exec.query(`SELECT 1 FROM pg_catalog.pg_roles r CROSS JOIN unnest($2::text[]) table_name CROSS JOIN unnest(ARRAY['INSERT','UPDATE','DELETE','TRUNCATE']) privilege WHERE pg_catalog.pg_has_role($1,r.oid,'MEMBER') AND pg_catalog.has_table_privilege(r.rolname,table_name,privilege) LIMIT 1`,[role,GOVERNED_DML_TABLES])).rows[0];if(authorityDml)throw new ContractValidationError('runtime role posture drifted to governed authority/evidence DML');
