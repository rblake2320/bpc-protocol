...
export interface SourceLeaseBinding { streamId: string; epoch: number; holderNodeId: string; leaseId: string; grantDigest: string; redisClaimDigest:string; maxClockSkewMs: number; activationDigest?:string }
...
export class PgSourceLeaseFence {
  private constructor(private readonly db: PgTransactor, private readonly resolver: PublicKeyResolver, private readonly binding: SourceLeaseBinding,private readonly quorum:BpcRedisQuorumFenceStore) {
    ...
    safeInt(binding.epoch, 'epoch', 1, MAX_EPOCH); safeInt(binding.maxClockSkewMs, 'maxClockSkewMs', 0, 60_000);
  ...
  async assertWritableInTx(exec: PgExecutor): Promise<void> {
    ...
    if(this.binding.epoch>1){const active=(await exec.query('SELECT state_digest_signed,receipt FROM bpc_ha.source_activation WHERE stream_id=$1 AND epoch=$2',[this.binding.streamId,this.binding.epoch])).rows[0];if(!active||String(active.state_digest_signed)!==this.binding.activationDigest)throw new ContractValidationError('promoted source ACTIVE receipt missing or stale');const receipt=active.receipt as CutoverReceipt;verifyCutoverReceipt(this.resolver,receipt);if(receipt.phase!=='ACTIVE'||receipt.streamId!==this.binding.streamId||receipt.targetEpoch!==this.binding.epoch||receipt.stateDigestSigned!==this.binding.activationDigest)throw new ContractValidationError('promoted source ACTIVE receipt binding invalid');}
...
export interface RedisFenceRecord { streamId: string; epoch: number; nodeId: string; commandId: string; claimedAtMs: number; keyId: string; signature: string }
...
function redisFenceMessage(value: BareRedisFenceRecord): Buffer {
  return frame('bpc-redis-fence/v1', value.streamId, value.epoch, value.nodeId, value.commandId, value.claimedAtMs);
}
export function signRedisFenceRecord(keyId: string, key: KeyObject | string, bare: BareRedisFenceRecord): RedisFenceRecord {
  id(bare.streamId, STREAM_RE, 'streamId'); id(bare.nodeId, ID_RE, 'nodeId'); id(bare.commandId, ID_RE, 'commandId');
...
export class BpcRedisFenceStore {
  ...
  private parse(raw: string): RedisFenceRecord {
    ...
    id(v.streamId, STREAM_RE, 'streamId'); id(v.nodeId, ID_RE, 'nodeId'); id(v.commandId, ID_RE, 'commandId');
...
export interface PromotionReadinessAttestation{streamId:string;commandId:string;targetEpoch:number;targetSourceEpoch:string;manifestDigest:string;appliedSequence:number;stateDigest:string;fencedDigest:string;keyId:string;attestationDigest:string;signature:string}
type BarePromotionAttestation=Omit<PromotionReadinessAttestation,'keyId'|'attestationDigest'|'signature'>;
const promotionAttestationMessage=(v:BarePromotionAttestation,d:string)=>frame('bpc-promotion-readiness/v1',v.streamId,v.commandId,v.targetEpoch,v.targetSourceEpoch,v.manifestDigest,v.appliedSequence,v.stateDigest,v.fencedDigest,d);
export function verifyPromotionReadinessAttestation(resolver:PublicKeyResolver,v:PromotionReadinessAttestation):void{id(v.streamId,STREAM_RE,'streamId');id(v.commandId,ID_RE,'commandId');id(v.targetSourceEpoch,ID_RE,'targetSourceEpoch');safeInt(v.targetEpoch,'targetEpoch',1,MAX_EPOCH);safeInt(v.appliedSequence,'appliedSequence');for(const x of [v.manifestDigest,v.stateDigest,v.fencedDigest,v.attestationDigest])if(!HEX64.test(x))throw new ContractValidationError('promotion attestation digest malformed');if(sha256(promotionAttestationMessage(v,''))!==v.attestationDigest)throw new ContractValidationError('promotion attestation digest mismatch');verify(resolver,v.keyId,promotionAttestationMessage(v,v.attestationDigest),v.signature);}
export async function buildPromotionReadinessAttestation(db:PgTransactor,fenced:CutoverReceipt,keyId:string,key:KeyObject|string):Promise<PromotionReadinessAttestation>{if(fenced.phase!=='FENCED')throw new ContractValidationError('readiness attestation requires FENCED cutover');return db.transaction(async exec=>{const cp=(await exec.query('SELECT source_epoch,sequence FROM ha_outbox_receiver_checkpoint WHERE stream_id=$1',[fenced.streamId])).rows[0];const source=(await exec.query('SELECT source_epoch,sequence FROM ha_outbox_source_checkpoint WHERE stream_id=$1',[fenced.streamId])).rows[0];if(!cp||safeInt(cp.sequence,'appliedSequence')!==fenced.finalSourceSequence||!source||String(source.source_epoch)!==fenced.targetSourceEpoch||safeInt(source.sequence,'newSourceSequence')!==0||await stateDigest(exec)!==fenced.stateDigest)throw new ContractValidationError('promoted receiver/source state is not ready');const bare:BarePromotionAttestation={streamId:fenced.streamId,commandId:fenced.commandId,targetEpoch:fenced.targetEpoch,targetSourceEpoch:fenced.targetSourceEpoch,manifestDigest:fenced.manifestDigest,appliedSequence:fenced.finalSourceSequence,stateDigest:fenced.stateDigest,fencedDigest:fenced.stateDigestSigned};const attestationDigest=sha256(promotionAttestationMessage(bare,''));return{...bare,keyId,attestationDigest,signature:sign(keyId,key,promotionAttestationMessage(bare,attestationDigest))};});}
export interface CutoverReceipt {
  ...
  stateDigest:string; redisClaimDigest:string; oldLeaseDigest:string; oldLeaseExpiresAtMs:number;
...
const cutoverMessage=(v:BareCutover,digest:string):Buffer=>frame('bpc-cutover/v1',v.streamId,v.commandId,v.phase,v.previousEpoch,v.targetEpoch,v.targetNodeId,v.targetSourceEpoch,v.manifestDigest,v.finalSourceSequence,v.stateDigest,v.redisClaimDigest,v.oldLeaseDigest,v.oldLeaseExpiresAtMs,v.prevStateDigest,digest);
function validateBareCutover(v:BareCutover):void{
  ...
  safeInt(v.previousEpoch,'previousEpoch',0,MAX_EPOCH-1);safeInt(v.targetEpoch,'targetEpoch',1,MAX_EPOCH);safeInt(v.finalSourceSequence,'finalSourceSequence');safeInt(v.oldLeaseExpiresAtMs,'oldLeaseExpiresAtMs',0,MAX_MS);
...
export class PgRedisFenceWitness{
  ...
  async verify(record:RedisFenceRecord|null):Promise<void>{if(!record)throw new ContractValidationError('empty Redis fence state is not authoritative');await this.db.transaction(async exec=>{const witness=(await exec.query('SELECT epoch,state_digest FROM bpc_ha.epoch_witness WHERE stream_id=$1 FOR SHARE',[record.streamId])).rows[0];const epoch=witness?safeInt(witness.epoch,'witnessEpoch',0,MAX_EPOCH):0;if(epoch===0){if(record.epoch!==1)throw new ContractValidationError('Redis genesis must begin at epoch 1');return;}
...
export function pairSnapshotManifestDigest(manifest:PairSnapshotManifest):string{return sha256(snapshotMessage(manifest));}
...
export async function promoteReceiverToSource(
  ...
  const manifestDigest=sha256(snapshotMessage(manifest));
