...
CREATE OR REPLACE FUNCTION bpc_ha.reserve_outbox_append(ticket jsonb,max_pending bigint,requested_fence numeric,mutation jsonb)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog AS $fn$
DECLARE k record; l record; cp record; pending bigint; now_ms bigint; message text; reservation_digest text; policy_digest text;
BEGIN
  IF max_pending<1 OR (SELECT count(*) FROM jsonb_object_keys(ticket))<>15 OR ticket->>'action'<>'outbox-reserve' OR ticket->>'mac' !~ '^[0-9a-f]{64}$' THEN RAISE EXCEPTION 'invalid authenticated outbox reservation'; END IF;
  ...
  IF ticket->>'txid'<>pg_catalog.txid_current()::text OR (ticket->>'expiresAtMs')::bigint<now_ms OR (ticket->>'expiresAtMs')::bigint>=l.expires_at_ms THEN RAISE EXCEPTION 'reservation ticket or lease expired'; END IF;
  reservation_digest:=encode(public.digest(convert_to(jsonb_build_array(requested_fence::text,max_pending::text,mutation)::text,'UTF8'),'sha256'),'hex');
  IF reservation_digest<>ticket->>'payloadDigest' THEN RAISE EXCEPTION 'reservation payload mismatch'; END IF;
  policy_digest:=encode(public.digest(convert_to(jsonb_build_object('action','outbox-reserve','mutation',mutation,'payload',jsonb_build_array(requested_fence::text,max_pending::text,mutation),'streamId',ticket->>'streamId','epoch',ticket->>'epoch','leaseId',ticket->>'leaseId','grantDigest',ticket->>'grantDigest','holderNodeId',l.holder_node_id,'leaseExpiresAtMs',l.expires_at_ms::text)::text,'UTF8'),'sha256'),'hex');
  IF policy_digest<>ticket->>'policyDigest' THEN RAISE EXCEPTION 'reservation policy mismatch'; END IF;
  ...
  IF NOT bpc_ha.constant_time_equal(public.hmac(convert_to(message,'UTF8'),k.secret,'sha256'),decode(ticket->>'mac','hex')) THEN RAISE EXCEPTION 'invalid reservation ticket MAC'; END IF;
  INSERT INTO bpc_ha.mutation_ticket_nonce(nonce,expires_at_ms) VALUES(ticket->>'nonce',(ticket->>'expiresAtMs')::bigint);
  PERFORM 1 FROM public.ha_outbox_fence WHERE stream_id=ticket->>'streamId' AND fence_token=requested_fence FOR UPDATE;
  IF NOT FOUND THEN RAISE EXCEPTION 'stream fence missing or stale'; END IF;
  ...
  SELECT count(*) INTO pending FROM public.ha_outbox_rows WHERE stream_id=ticket->>'streamId' AND acked_at IS NULL AND quarantined_at IS NULL;
  IF pending>=max_pending THEN RAISE EXCEPTION 'authoritative outbox backpressure'; END IF;
  RETURN jsonb_build_object('fenceToken',requested_fence::text,'sourceEpoch',cp.source_epoch,'sequence',cp.sequence+1);
END $fn$;
REVOKE ALL ON FUNCTION bpc_ha.reserve_outbox_append(jsonb,bigint,numeric,jsonb) FROM PUBLIC;
...
DECLARE k record; l record; now_ms bigint; message text; mutation_digest text; policy_digest text;
BEGIN
  IF (SELECT count(*) FROM jsonb_object_keys(ticket))<>15 OR ticket->>'action'<>'outbox-append' OR ticket->>'mac' !~ '^[0-9a-f]{64}$' OR ticket->>'opDigest' !~ '^[0-9a-f]{64}$' THEN RAISE EXCEPTION 'invalid outbox ticket'; END IF;
  SELECT * INTO k FROM bpc_ha.mutation_ticket_key WHERE key_id=ticket->>'keyId' AND active FOR SHARE; IF NOT FOUND THEN RAISE EXCEPTION 'unknown ticket key'; END IF;
  SELECT * INTO l FROM bpc_ha.source_lease WHERE stream_id=ticket->>'streamId' FOR SHARE; IF NOT FOUND OR l.status<>'active' OR l.epoch::text<>ticket->>'epoch' OR l.lease_id<>ticket->>'leaseId' OR l.grant_digest<>ticket->>'grantDigest' THEN RAISE EXCEPTION 'stale source lease'; END IF;
  now_ms:=floor(extract(epoch FROM pg_catalog.clock_timestamp())*1000)::bigint;
  ...
  policy_digest:=encode(public.digest(convert_to(jsonb_build_object('action','outbox-append','mutation',mutation,'payload',jsonb_build_array(fence_value::text,mutation),'streamId',ticket->>'streamId','epoch',ticket->>'epoch','leaseId',ticket->>'leaseId','grantDigest',ticket->>'grantDigest','holderNodeId',l.holder_node_id,'leaseExpiresAtMs',l.expires_at_ms::text)::text,'UTF8'),'sha256'),'hex');IF policy_digest<>ticket->>'policyDigest' THEN RAISE EXCEPTION 'outbox policy mismatch'; END IF;
  message:=concat_ws('|','bpc-db-mutation/v1',ticket->>'keyId',ticket->>'nonce',ticket->>'streamId',ticket->>'epoch',ticket->>'leaseId',ticket->>'grantDigest',ticket->>'txid',ticket->>'expiresAtMs',ticket->>'sourceEpoch',ticket->>'sequence',ticket->>'opDigest',ticket->>'action',ticket->>'payloadDigest',ticket->>'policyDigest');
...
AS $fn$
DECLARE
  k record; l record; affected bigint; now_ms bigint; message text; outbox_mutation jsonb; policy_digest text;
BEGIN
  ...
  SELECT r.mutation INTO outbox_mutation FROM public.ha_outbox_rows r WHERE r.stream_id=ticket->>'streamId' AND r.source_epoch=ticket->>'sourceEpoch' AND r.sequence=(ticket->>'sequence')::bigint AND r.op_digest=ticket->>'opDigest' AND r.xmin::text=(((ticket->>'txid')::bigint % 4294967296)::text);IF NOT FOUND THEN RAISE EXCEPTION 'mutation ticket is not coupled to a current-transaction outbox row'; END IF;
  policy_digest:=encode(public.digest(convert_to(jsonb_build_object('action',action,'mutation',outbox_mutation,'payload',payload,'streamId',ticket->>'streamId','epoch',ticket->>'epoch','leaseId',ticket->>'leaseId','grantDigest',ticket->>'grantDigest','holderNodeId',l.holder_node_id,'leaseExpiresAtMs',l.expires_at_ms::text)::text,'UTF8'),'sha256'),'hex');IF policy_digest<>ticket->>'policyDigest' THEN RAISE EXCEPTION 'mutation policy mismatch'; END IF;
  message := concat_ws('|','bpc-db-mutation/v1',ticket->>'keyId',ticket->>'nonce',ticket->>'streamId',ticket->>'epoch',ticket->>'leaseId',ticket->>'grantDigest',ticket->>'txid',ticket->>'expiresAtMs',ticket->>'sourceEpoch',ticket->>'sequence',ticket->>'opDigest',ticket->>'action',ticket->>'payloadDigest',ticket->>'policyDigest');
...
export const BPC_HA_SCHEMA_MANIFEST='09e8504a1f568c9559306df0f21d8069ccc053c2028502594b11957a4e889295';
...
export class PgSourceLeaseFence {
  ...
  private async issueTicket(exec:PgExecutor,header:OutboxRecordHeader,action:string,payloadJson:string,mutationJson:string):Promise<Record<string,string>>{
    ...
    const canonicalPolicy=String(clock?.policy_canonical),policyDigest=sha256(canonicalPolicy);const request=Object.freeze({domain:'bpc-db-mutation/v1' as const,keyId:this.mutationSigner.keyId,nonce,streamId:this.binding.streamId,epoch:String(this.binding.epoch),leaseId:this.binding.leaseId,grantDigest:this.binding.grantDigest,txid,expiresAtMs:String(expiresAtMs),sourceEpoch:header.sourceEpoch,sequence:String(header.sequence),opDigest:header.opDigest,action,payloadDigest,policyDigest});const mac=await this.mutationSigner.signTicket(request,Object.freeze({canonicalPolicy}));if(!/^[0-9a-f]{64}$/.test(mac))throw new ContractValidationError('mutation ticket signer returned invalid MAC');
  ...
  async appendGovernedOutbox(exec:PgExecutor,input:{streamId:string;mutation:SanitizedMutation<unknown>;fenceToken:bigint;maxPendingRows:number}):Promise<OutboxRecordHeader>{
    await this.assertWritableInTx(exec);if(input.streamId!==this.binding.streamId)throw new ContractValidationError('governed outbox stream mismatch');const fenceDecimal=String(input.fenceToken),mutationJson=JSON.stringify(input.mutation);const reservationPayload=JSON.stringify([fenceDecimal,String(input.maxPendingRows),input.mutation]);const reservationHeader:OutboxRecordHeader={contractVersion:'1',streamId:input.streamId,sourceEpoch:'reservation',sequence:0,fenceToken:fenceDecimal,opDigest:sha256(reservationPayload)};const reservationTicket=await this.issueTicket(exec,reservationHeader,'outbox-reserve',reservationPayload,mutationJson);const reserved=(await exec.query('SELECT bpc_ha.reserve_outbox_append($1::jsonb,$2,$3::numeric,$4::jsonb) value',[JSON.stringify(reservationTicket),input.maxPendingRows,fenceDecimal,mutationJson])).rows[0]?.value as Record<string,unknown>|undefined;if(!reserved)throw new ContractValidationError('governed outbox reservation failed');if(String(reserved.fenceToken)!==fenceDecimal)throw new ContractValidationError('governed outbox fence mismatch');const sourceEpoch=String(reserved.sourceEpoch),sequence=safeInt(reserved.sequence,'reserved outbox sequence',1);const opDigest=canonicalOpDigest({streamId:input.streamId,sourceEpoch,sequence,fenceToken:fenceDecimal,mutation:input.mutation});const header:OutboxRecordHeader={contractVersion:'1',streamId:input.streamId,sourceEpoch,sequence,fenceToken:fenceDecimal,opDigest};assertHeaderConformant(header);const payloadJson=JSON.stringify([fenceDecimal,input.mutation]);const ticket=await this.issueTicket(exec,header,'outbox-append',payloadJson,mutationJson);await exec.query('SELECT bpc_ha.append_governed_outbox($1::jsonb,$2::numeric,$3::jsonb)',[JSON.stringify(ticket),fenceDecimal,mutationJson]);return header;
  }
...
export async function provisionBpcRuntimeMutationBoundary(db:PgTransactor,runtimeRole:string,keyId:string,secret:Uint8Array):Promise<void>{
  ...
  try{await db.transaction(async exec=>{const role=(await exec.query('SELECT rolsuper,rolbypassrls,rolinherit FROM pg_catalog.pg_roles WHERE rolname=$1',[runtimeRole])).rows[0];if(!role||role.rolsuper||role.rolbypassrls)throw new ContractValidationError('runtime role missing or holds bypass authority');
    ...
    await exec.query(`GRANT EXECUTE ON FUNCTION bpc_ha.reserve_outbox_append(jsonb,bigint,numeric,jsonb),bpc_ha.append_governed_outbox(jsonb,numeric,jsonb) TO ${runtimeRole}`);
    ...
    const authorityDml=(await exec.query(`SELECT 1 FROM pg_catalog.pg_roles r WHERE pg_catalog.pg_has_role($1,r.oid,'MEMBER') AND (pg_catalog.has_table_privilege(r.rolname,'public.ha_outbox_source_checkpoint','INSERT') OR pg_catalog.has_table_privilege(r.rolname,'public.ha_outbox_source_checkpoint','DELETE') OR pg_catalog.has_table_privilege(r.rolname,'public.ha_outbox_fence','INSERT') OR pg_catalog.has_table_privilege(r.rolname,'public.ha_outbox_fence','DELETE')) LIMIT 1`,[runtimeRole])).rows[0];if(authorityDml)throw new ContractValidationError('runtime role membership retains checkpoint/fence INSERT or DELETE');
...
async function assertBpcRuntimeMutationBoundaryExec(exec:PgExecutor,expectedRole?:string):Promise<string>{
  ...
  const authorityDml=(await exec.query(`SELECT 1 FROM pg_catalog.pg_roles r WHERE pg_catalog.pg_has_role($1,r.oid,'MEMBER') AND (pg_catalog.has_table_privilege(r.rolname,'public.ha_outbox_source_checkpoint','INSERT') OR pg_catalog.has_table_privilege(r.rolname,'public.ha_outbox_source_checkpoint','DELETE') OR pg_catalog.has_table_privilege(r.rolname,'public.ha_outbox_fence','INSERT') OR pg_catalog.has_table_privilege(r.rolname,'public.ha_outbox_fence','DELETE')) LIMIT 1`,[role])).rows[0];if(authorityDml)throw new ContractValidationError('runtime role posture drifted to checkpoint/fence INSERT or DELETE');
