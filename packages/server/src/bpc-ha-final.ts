...
export function validateDbMutationPolicyContext(request:Readonly<DbMutationTicketRequest>,context:Readonly<DbMutationPolicyContext>):void{
  ...
  const kind=String(mutation['kind']),id0=String(payload[0]??'');let allowed=false;
  if(kind==='bpc.pair.set.v1')allowed=request.action==='outbox-append'||request.action==='pair-upsert'&&id0===String(mutation['pairId']);
  else if(kind==='bpc.pair.delete.v1')allowed=request.action==='outbox-append'||request.action==='pair-delete'&&id0===String(mutation['pairId']);
  else if(kind==='bpc.pending.set.v1')allowed=request.action==='outbox-append'||request.action==='pending-upsert'&&id0===String(mutation['token'])&&String(payload[2])===String(mutation['requestedAt']);
  else if(kind==='bpc.pending.delete.v1')allowed=request.action==='outbox-append'||request.action==='pending-delete'&&id0===String(mutation['token']);
  else if(kind==='bpc.pair.approve.v1')allowed=request.action==='outbox-append'||request.action==='pending-delete'&&id0===String(mutation['token'])||request.action==='pair-insert'&&id0===String(mutation['pairId']);
  else if(kind==='bpc.pair.rotate.v1')allowed=request.action==='outbox-append'||request.action==='pair-upsert'&&(id0===String(mutation['oldPairId'])||id0===String(mutation['newPairId']));
...
export class PgSourceLeaseFence {
  ...
  private async issueTicket(exec:PgExecutor,header:OutboxRecordHeader,action:string,payloadJson:string,mutationJson:string):Promise<Record<string,string>>{
    ...
    const canonicalPolicy=String(clock?.policy_canonical),policyDigest=sha256(canonicalPolicy);const request=Object.freeze({domain:'bpc-db-mutation/v1' as const,keyId:this.mutationSigner.keyId,nonce,streamId:this.binding.streamId,epoch:String(this.binding.epoch),leaseId:this.binding.leaseId,grantDigest:this.binding.grantDigest,txid,expiresAtMs:String(expiresAtMs),sourceEpoch:header.sourceEpoch,sequence:String(header.sequence),opDigest:header.opDigest,action,payloadDigest,policyDigest}),context=Object.freeze({canonicalPolicy});validateDbMutationPolicyContext(request,context);const mac=await this.mutationSigner.signTicket(request,context);if(!/^[0-9a-f]{64}$/.test(mac))throw new ContractValidationError('mutation ticket signer returned invalid MAC');
...
export async function provisionBpcRuntimeMutationBoundary(db:PgTransactor,runtimeRole:string,keyId:string,secret:Uint8Array):Promise<void>{
  ...
  try{await db.transaction(async exec=>{const role=(await exec.query('SELECT rolsuper,rolbypassrls,rolinherit FROM pg_catalog.pg_roles WHERE rolname=$1',[runtimeRole])).rows[0];if(!role||role.rolsuper||role.rolbypassrls)throw new ContractValidationError('runtime role missing or holds bypass authority');
    ...
    const authorityDml=(await exec.query(`SELECT 1 FROM pg_catalog.pg_roles r CROSS JOIN unnest(ARRAY['ha_outbox_meta','ha_outbox_fence','ha_outbox_source_checkpoint','ha_outbox_receiver_checkpoint','ha_outbox_rows','ha_outbox_publisher_lease','ha_outbox_quarantine','ha_outbox_applied']) table_name CROSS JOIN unnest(ARRAY['INSERT','UPDATE','DELETE']) privilege WHERE pg_catalog.pg_has_role($1,r.oid,'MEMBER') AND pg_catalog.has_table_privilege(r.rolname,pg_catalog.format('public.%I',table_name),privilege) LIMIT 1`,[runtimeRole])).rows[0];if(authorityDml)throw new ContractValidationError('runtime role membership retains governed outbox/evidence DML');
...
async function assertBpcRuntimeMutationBoundaryExec(exec:PgExecutor,expectedRole?:string):Promise<string>{
  ...
  const authorityDml=(await exec.query(`SELECT 1 FROM pg_catalog.pg_roles r CROSS JOIN unnest(ARRAY['ha_outbox_meta','ha_outbox_fence','ha_outbox_source_checkpoint','ha_outbox_receiver_checkpoint','ha_outbox_rows','ha_outbox_publisher_lease','ha_outbox_quarantine','ha_outbox_applied']) table_name CROSS JOIN unnest(ARRAY['INSERT','UPDATE','DELETE']) privilege WHERE pg_catalog.pg_has_role($1,r.oid,'MEMBER') AND pg_catalog.has_table_privilege(r.rolname,pg_catalog.format('public.%I',table_name),privilege) LIMIT 1`,[role])).rows[0];if(authorityDml)throw new ContractValidationError('runtime role posture drifted to governed outbox/evidence DML');
