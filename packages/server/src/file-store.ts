...
function validPair(v:unknown,key:string):v is StoredPair{if(!obj(v)||own(v,'id')!==key||!validRegistration(v)||!(own(v,'status')==='active'||own(v,'status')==='locked'||own(v,'status')==='expired'||own(v,'status')==='rotated'||own(v,'status')==='revoked')||!natural(own(v,'created'))||!(own(v,'lastActive')===null||natural(own(v,'lastActive')))||!natural(own(v,'requests'))||!natural(own(v,'failedSigs')))return false;const score=own(v,'cumulativeFailures');if(score!==undefined&&score!==null&&!(typeof score==='number'&&Number.isFinite(score)&&score>=0))return false;const first=own(v,'firstFailureAt');return first===undefined||first===null||natural(first);}
...
export class FilePairStore extends AtomicFile<FileStoreData> implements PairStore{
 ...
 async get(id:string){return this.tx(d=>({result:Object.hasOwn(d.pairs,id)?structuredClone(d.pairs[id]):undefined}));}
 ...
 async getPending(token:string){return this.tx(d=>({result:Object.hasOwn(d.pending,token)?structuredClone(d.pending[token]):undefined}));}
