...
function assertApprovalPair(registration:CanonicalPairRegistration,pair:CanonicalStoredPair):void{if(pair.status!=='active'||pair.requests!==0||pair.failedSigs!==0||pair.lastActive!==null||pair.cumulativeFailures!==undefined||pair.firstFailureAt!==undefined||!sameCanonical(registrationFromPairObject(pair as unknown as Record<string,unknown>),registration))throw new ContractValidationError('approved pair does not match pending registration/initial state');}
function rotationPolicy(pair:CanonicalStoredPair):object{return{name:pair.name,scope:pair.scope,mode:pair.mode,secretHash:pair.secretHash,expiresAt:pair.expiresAt??null,maxRequests:pair.maxRequests??null,kind:pair.kind??'legitimate',canaryClass:pair.canaryClass??null};}
...
export class PgTransactionalPairStore implements AtomicPairStore {
  ...
  async rotatePair(expectedOldValue:StoredPair,replacementValue:StoredPair):Promise<boolean>{
    ...
    if(expectedOld.status!=='active'||replacement.status!=='active'||replacement.requests!==0||replacement.failedSigs!==0||replacement.lastActive!==null||!sameCanonical(rotationPolicy(expectedOld),rotationPolicy(replacement)))throw new ContractValidationError('rotation state/policy inheritance invalid');
...
export class PgPairMutationApplier implements MutationApplier<BpcPairMutation>{private readonly codec:Aes256GcmPairPayloadCodec;constructor(private readonly streamId:string,keyring:PairSealKeyring){if(!streamId||streamId.length>512)throw new ContractValidationError('streamId invalid');this.codec=new Aes256GcmPairPayloadCodec(keyring.activeKeyId,keyring.resolveKey);}async applyInTx(exec:PgExecutor,record:OutboxRecord<BpcPairMutation>){if(record.streamId!==this.streamId)throw new ContractValidationError('pair mutation stream mismatch');const m=normalizeMutation(record.mutation) as BpcPairMutation;switch(m.kind){case'bpc.pair.set.v1':await upsertPair(exec,this.codec.openPair(m.sealed,{domain:'bpc-pair-payload',version:'1',streamId:this.streamId,kind:m.kind,pairId:m.pairId}));return;case'bpc.pair.delete.v1':affectedZeroOrOne(await exec.query('DELETE FROM bpc_pairs WHERE id=$1',[m.pairId]),'pair delete');return;case'bpc.pending.set.v1':await upsertPending(exec,m.token,this.codec.openRegistration(m.sealed,{domain:'bpc-pair-payload',version:'1',streamId:this.streamId,kind:m.kind,token:m.token,requestedAt:m.requestedAt}),m.requestedAt);return;case'bpc.pending.delete.v1':affectedZeroOrOne(await exec.query('DELETE FROM bpc_pending WHERE token=$1',[m.token]),'pending delete');return;case'bpc.pair.approve.v1':{const pending=this.codec.openRegistration(m.expectedPending,{domain:'bpc-pair-payload',version:'1',streamId:this.streamId,kind:m.kind,token:m.token,requestedAt:m.requestedAt}),approved=this.codec.openPair(m.sealed,{domain:'bpc-pair-payload',version:'1',streamId:this.streamId,kind:m.kind,pairId:m.pairId});assertApprovalPair(pending,approved);const row=(await exec.query('SELECT registration,requested_at FROM bpc_pending WHERE token=$1 FOR UPDATE',[m.token])).rows[0];if(!row||Number(row['requested_at'])!==m.requestedAt||!sameCanonical(normalizeRegistration(row['registration']),pending))throw new ContractValidationError('replica pending approval precondition failed');if((await exec.query('SELECT 1 FROM bpc_pairs WHERE id=$1',[m.pairId])).rows.length)throw new ContractValidationError('replica approval pair identity already exists');affectedOne(await exec.query('DELETE FROM bpc_pending WHERE token=$1 AND requested_at=$2',[m.token,m.requestedAt]),'replica pending approval delete');await insertPair(exec,approved);return;}case'bpc.pair.rotate.v1':{const expected=this.codec.openPair(m.expectedOld,{domain:'bpc-pair-payload',version:'1',streamId:this.streamId,kind:m.kind,pairId:m.oldPairId}),replacement=this.codec.openPair(m.sealed,{domain:'bpc-pair-payload',version:'1',streamId:this.streamId,kind:m.kind,pairId:m.newPairId});if(replacement.status!=='active'||replacement.requests!==0||replacement.failedSigs!==0||replacement.lastActive!==null||!sameCanonical(rotationPolicy(expected),rotationPolicy(replacement)))throw new ContractValidationError('replica rotation policy inheritance failed');const row=(await exec.query('SELECT * FROM bpc_pairs WHERE id=$1 FOR UPDATE',[m.oldPairId])).rows[0];if(!row||!sameCanonical(normalizePair(rowToPair(row),false),expected))throw new ContractValidationError('replica rotation precondition failed');if((await exec.query('SELECT 1 FROM bpc_pairs WHERE id=$1',[m.newPairId])).rows.length)throw new ContractValidationError('replica rotation pair identity already exists');await upsertPair(exec,{...expected,status:'rotated'});await insertPair(exec,replacement);return;}}}}
