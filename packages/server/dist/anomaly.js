...
export class AnomalyEngine {
    ...
    async recordRequest(pairId) {
        ...
        if (pairId)
            await this.store.increment(`pair:${pairId}:total`, WINDOW_MS);
    }
    async recordDenied(pairId) {
        await this.store.increment('global:denied', WINDOW_MS);
        if (pairId)
            await this.store.increment(`pair:${pairId}:denied`, WINDOW_MS);
    }
    async recordUnknownPair() {
    ...
    async recordSigFailure(pairId) {
        ...
        if (pairId)
            await this.store.increment(`pair:${pairId}:sig_fail`, WINDOW_MS);
    }
    async recordReplay(pairId) {
        await this.store.increment('global:replay', WINDOW_MS);
        if (pairId)
            await this.store.increment(`pair:${pairId}:replay`, WINDOW_MS);
    }
    async recordExpiredTimestamp(pairId) {
        await this.store.increment('global:expired_ts', WINDOW_MS);
        if (pairId)
            await this.store.increment(`pair:${pairId}:expired_ts`, WINDOW_MS);
    }
    // ─────────────────────────────────────────────────────────────────────────
     ...
     */
    isInShadowState(pairId, sourceIp) {
