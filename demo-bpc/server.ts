/**
 * BPC Demo Server (Layers 1-5)
 *
 * Real backend wired to @bpc/server — serves the demo UI and validates
 * every request through the BPC verification pipeline (Layers 1-5).
 *
...
// NEVER point BPC_DATA_DIR at %APPDATA%\SelfConnect\ — that is enterprise territory.
...
server.listen(PORT, () => {
  console.log(`\nBPC Demo Server (Layers 1-5) running on http://localhost:${PORT}`);
