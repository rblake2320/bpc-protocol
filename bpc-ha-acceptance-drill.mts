...
let lastAppendTicket:Record<string,string>|undefined;const mutationTicketSecret=Buffer.alloc(32,59);const mutationTicketSigner={keyId:'mutation-v1',async signTicket(request:Record<string,string>,context:{canonicalPolicy:string}){validateDbMutationPolicyContext(request as never,context,new Aes256GcmPairPayloadCodec('seal-v1',keyring.resolveKey));const mac=createHmac('sha256',mutationTicketSecret).update([request.domain,request.keyId,request.nonce,request.streamId,request.epoch,request.leaseId,request.grantDigest,request.txid,request.expiresAtMs,request.sourceEpoch,request.sequence,request.opDigest,request.action,request.maxPendingRows,request.payloadDigest,request.policyDigest].join('|')).digest('hex');if(request.action==='outbox-append'){const{domain:_domain,...fields}=request;lastAppendTicket={...fields,mac};}return mac;}};
...
const pairPayload=(p:ReturnType<typeof pair>)=>[p.id,p.name,p.scope,p.mode,p.secretHash,JSON.stringify(p.pubJwk),p.status,p.created,p.lastActive,p.requests,p.failedSigs,null,null,null,'legitimate',null,null];
...
async function main(): Promise<void> {
  const policyMutation={kind:'bpc.pair.delete.v1',pairId:'pair-a'},wrongIdentityPolicy=JSON.stringify({action:'pair-delete',mutation:policyMutation,payload:['pair-b'],streamId:'policy-stream',sourceEpoch:'policy-epoch',sequence:'1',fenceToken:'1',maxPendingRows:'0'}),policyHeader={domain:'bpc-db-mutation/v1',keyId:'mutation-v1',nonce:'n',streamId:'policy-stream',epoch:'1',leaseId:'l',grantDigest:'0'.repeat(64),txid:'1',expiresAtMs:'1',sourceEpoch:'policy-epoch',sequence:'1',opDigest:canonicalOpDigest({streamId:'policy-stream',sourceEpoch:'policy-epoch',sequence:1,fenceToken:'1',mutation:policyMutation as never}),action:'pair-delete',maxPendingRows:'0',payloadDigest:'0'.repeat(64),policyDigest:createHash('sha256').update(wrongIdentityPolicy).digest('hex')},policyCodec=new Aes256GcmPairPayloadCodec('seal-v1',keyring.resolveKey);assert.throws(()=>validateDbMutationPolicyContext(policyHeader as never,{canonicalPolicy:wrongIdentityPolicy},policyCodec),/complete DML payload/);const rightIdentityPolicy=wrongIdentityPolicy.replace('pair-b','pair-a');assert.throws(()=>validateDbMutationPolicyContext({...policyHeader,opDigest:'f'.repeat(64),policyDigest:createHash('sha256').update(rightIdentityPolicy).digest('hex')} as never,{canonicalPolicy:rightIdentityPolicy},policyCodec),/opDigest mismatch/);
  const sourcePair=pair(99),sealed=policyCodec.sealPair(sourcePair as never,{domain:'bpc-pair-payload',version:'1',streamId:'policy-stream',kind:'bpc.pair.set.v1',pairId:sourcePair.id}),setMutation={kind:'bpc.pair.set.v1',pairId:sourcePair.id,sealed},wrongFields=[sourcePair.id,'ATTACKER NAME',sourcePair.scope,sourcePair.mode,sourcePair.secretHash,JSON.stringify(sourcePair.pubJwk),sourcePair.status,sourcePair.created,sourcePair.lastActive,sourcePair.requests,sourcePair.failedSigs,null,null,null,'legitimate',null,null],wrongFieldsPolicy=JSON.stringify({action:'pair-upsert',mutation:setMutation,payload:wrongFields,streamId:'policy-stream',sourceEpoch:'policy-epoch',sequence:'1',fenceToken:'1',maxPendingRows:'0'}),setHeader={...policyHeader,action:'pair-upsert',opDigest:canonicalOpDigest({streamId:'policy-stream',sourceEpoch:'policy-epoch',sequence:1,fenceToken:'1',mutation:setMutation as never}),policyDigest:createHash('sha256').update(wrongFieldsPolicy).digest('hex')};assert.throws(()=>validateDbMutationPolicyContext(setHeader as never,{canonicalPolicy:wrongFieldsPolicy},policyCodec),/complete DML payload/);const malformedMutation={...policyMutation,extra:'forbidden'},malformedPolicy=JSON.stringify({action:'pair-delete',mutation:malformedMutation,payload:['pair-a'],streamId:'policy-stream',sourceEpoch:'policy-epoch',sequence:'1',fenceToken:'1',maxPendingRows:'0'});assert.throws(()=>validateDbMutationPolicyContext({...policyHeader,opDigest:canonicalOpDigest({streamId:'policy-stream',sourceEpoch:'policy-epoch',sequence:1,fenceToken:'1',mutation:malformedMutation as never}),policyDigest:createHash('sha256').update(malformedPolicy).digest('hex')} as never,{canonicalPolicy:malformedPolicy},policyCodec),/unexpected field/);
  ...
  assert.ok(lastAppendTicket);const committedMutation=(await poolA.query('SELECT mutation FROM ha_outbox_rows WHERE stream_id=$1 AND sequence=1',[SID])).rows[0].mutation;await assert.rejects(poolRuntimeA.query('SELECT bpc_ha.append_governed_outbox($1::jsonb,$2,$3::numeric,$4::jsonb)',[JSON.stringify(lastAppendTicket),101,1,JSON.stringify(committedMutation)]),/admission limit mismatch/);
  const directFence=await PgSourceLeaseFence.open(dbRuntimeA,haReadyRuntimeA,sourceResolver,binding1,fenceStore,nodeAIdentity,mutationTicketSigner),directOutbox=new PgDurableOutbox(dbRuntimeA,readyRuntimeA,{streamId:SID,sanitizer:bpcPairMutationSanitizer,maxPendingRows:100,backpressure:'fail-authoritative-mutation',preCommitCheck:(exec)=>directFence.assertWritableInTx(exec),governedAppend:(exec,input)=>directFence.appendGovernedOutbox(exec,input)});
  const directComposite=(mutation:BpcPairMutation,action:string,payload:unknown[])=>directOutbox.withOutboxTx(async(tx,exec)=>{const header=await directOutbox.appendInTx(tx,{streamId:SID,rawMutation:mutation,fenceToken:1n});await directFence.applyControlledMutation(exec,header,action,payload);});
  const appendOnlyPair=pair(105),appendOnlyMutation:BpcPairMutation={kind:'bpc.pair.set.v1',pairId:appendOnlyPair.id,sealed:policyCodec.sealPair(appendOnlyPair as never,{domain:'bpc-pair-payload',version:'1',streamId:SID,kind:'bpc.pair.set.v1',pairId:appendOnlyPair.id})};await assert.rejects(directOutbox.withOutboxTx(async(tx,exec)=>{await directOutbox.appendInTx(tx,{streamId:SID,rawMutation:appendOnlyMutation,fenceToken:1n});}),/COMMIT was dispatched/);assert.equal(Number((await poolA.query('SELECT sequence FROM ha_outbox_source_checkpoint WHERE stream_id=$1',[SID])).rows[0].sequence),1);assert.equal(Number((await poolA.query("SELECT count(*)::int n FROM ha_outbox_rows WHERE stream_id=$1 AND mutation->>'pairId'='pair-105'",[SID])).rows[0].n),0);assert.equal(Number((await poolA.query("SELECT count(*)::int n FROM bpc_pairs WHERE id='pair-105'")).rows[0].n),0);
  const wrongOld={...pair(1),name:'Wrong current state'},wrongReplacement={...wrongOld,id:'pair-evil-rotate',pubJwk:pair(103).pubJwk,created:pair(103).created},wrongRotate:BpcPairMutation={kind:'bpc.pair.rotate.v1',oldPairId:wrongOld.id,expectedOld:policyCodec.sealPair(wrongOld as never,{domain:'bpc-pair-payload',version:'1',streamId:SID,kind:'bpc.pair.rotate.v1',pairId:wrongOld.id}),newPairId:wrongReplacement.id,sealed:policyCodec.sealPair(wrongReplacement as never,{domain:'bpc-pair-payload',version:'1',streamId:SID,kind:'bpc.pair.rotate.v1',pairId:wrongReplacement.id})};
  await assert.rejects(directComposite(wrongRotate,'pair-rotate',[...pairPayload(wrongOld),...pairPayload(wrongReplacement)]),/rotation old-pair precondition/);assert.equal(Number((await poolA.query("SELECT count(*)::int n FROM bpc_pairs WHERE id='pair-evil-rotate'")).rows[0].n),0);
  const replacement={...pair(1),id:'pair-101',pubJwk:pair(101).pubJwk,created:pair(101).created};assert.equal(await storeA.rotatePair(pair(1),replacement),true);assert.equal(String((await poolA.query("SELECT status FROM bpc_pairs WHERE id='pair-1'")).rows[0].status),'rotated');assert.equal(String((await poolA.query("SELECT status FROM bpc_pairs WHERE id='pair-101'")).rows[0].status),'active');
  const approvedPair=pair(102),approvalRegistration={name:approvedPair.name,scope:approvedPair.scope,mode:approvedPair.mode,secretHash:approvedPair.secretHash,pubJwk:approvedPair.pubJwk};await storeA.setPending('approval-token',approvalRegistration,approvedPair.created);const wrongRegistration={...approvalRegistration,name:'Wrong pending state',kind:'legitimate' as const},wrongApproved={...pair(104),name:wrongRegistration.name,scope:wrongRegistration.scope,mode:wrongRegistration.mode,secretHash:wrongRegistration.secretHash,pubJwk:wrongRegistration.pubJwk},wrongApprove:BpcPairMutation={kind:'bpc.pair.approve.v1',token:'approval-token',requestedAt:approvedPair.created,expectedPending:policyCodec.sealRegistration(wrongRegistration as never,{domain:'bpc-pair-payload',version:'1',streamId:SID,kind:'bpc.pair.approve.v1',token:'approval-token',requestedAt:approvedPair.created}),pairId:wrongApproved.id,sealed:policyCodec.sealPair(wrongApproved as never,{domain:'bpc-pair-payload',version:'1',streamId:SID,kind:'bpc.pair.approve.v1',pairId:wrongApproved.id})};
  await assert.rejects(directComposite(wrongApprove,'pair-approve',['approval-token',JSON.stringify(wrongRegistration),approvedPair.created,100,...pairPayload(wrongApproved)]),/pending approval precondition/);assert.equal(Number((await poolA.query("SELECT count(*)::int n FROM bpc_pending WHERE token='approval-token'")).rows[0].n),1);assert.equal(await storeA.approvePending('approval-token',{registration:approvalRegistration,requestedAt:approvedPair.created},approvedPair,100),true);
  ...
  for(const [privilege,table] of [['TRUNCATE','public.ha_outbox_rows'],['TRUNCATE','public.bpc_pairs'],['TRUNCATE','public.bpc_pending'],['TRIGGER','public.bpc_pairs'],['UPDATE','bpc_ha.mutation_ticket_key'],['DELETE','bpc_ha.mutation_ticket_nonce'],['UPDATE','bpc_ha.source_lease']] as const){await poolA.query(`GRANT ${privilege} ON ${table} TO bpc_runtime_acceptance`);await assert.rejects(storeA.set(pair(93)),/posture drifted/);await poolA.query(`REVOKE ${privilege} ON ${table} FROM bpc_runtime_acceptance`);}
  ...
  await storeA.set(pair(2));const crashTargetSequence=Number((await poolA.query('SELECT sequence FROM ha_outbox_source_checkpoint WHERE stream_id=$1',[SID])).rows[0].sequence); holdAck=true; ackGate=new Promise<void>((resolve)=>{releaseAck=resolve;});
  const crashed=spawnWorker(workerEnv('publisher',receiverUrl)); let crashErr='';crashed.stderr!.on('data',(d)=>crashErr+=d);
  for(let i=0;i<100;i++){const seq=Number((await poolB.query('SELECT sequence FROM ha_outbox_receiver_checkpoint WHERE stream_id=$1',[SID])).rows[0].sequence);if(seq===crashTargetSequence)break;await sleep(25);if(i===99)throw new Error(`child never delivered before crash: ${crashErr}`);}
  ...
  assert.equal(Number((await poolB.query('SELECT count(*)::int n FROM ha_outbox_applied WHERE stream_id=$1',[SID])).rows[0].n),crashTargetSequence);

  await storeA.set(pair(3)); await drain();
  const snapshotSequence=Number((await poolA.query('SELECT sequence FROM ha_outbox_source_checkpoint WHERE stream_id=$1',[SID])).rows[0].sequence);
  // Capture C, discard the old receiver authority, and bootstrap a genuinely
  // fresh B solely from the signed source snapshot.
  const snapshotAtC=await buildPairSnapshotBundle<BpcPairMutation>(dbA,SID,snapshotSequence,'source-v1',sourcePrivate);
  ...
  assert.equal(Number((await poolB.query('SELECT sequence FROM ha_outbox_receiver_checkpoint WHERE stream_id=$1',[SID])).rows[0].sequence),snapshotSequence);
  ...
  const finalSourceSequence=Number((await poolA.query('SELECT sequence FROM ha_outbox_source_checkpoint WHERE stream_id=$1',[SID])).rows[0].sequence);
  const bundle=await buildPairSnapshotBundle<BpcPairMutation>(dbA,SID,finalSourceSequence,'source-v1',sourcePrivate);
  assert.equal(bundle.manifest.finalSequence,finalSourceSequence);
  ...
  assert.equal(Number((await poolB.query('SELECT sequence FROM ha_outbox_receiver_checkpoint WHERE stream_id=$1',[SID])).rows[0].sequence),finalSourceSequence);
