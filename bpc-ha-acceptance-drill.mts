...
async function main(): Promise<void> {
  ...
  await storeA.set(pair(1));assert.equal(Number((await poolA.query("SELECT count(*)::int n FROM bpc_pairs WHERE id='pair-1'")).rows[0].n),1);const replacement={...pair(1),id:'pair-101',pubJwk:pair(101).pubJwk,created:pair(101).created};assert.equal(await storeA.rotatePair(pair(1),replacement),true);assert.equal(String((await poolA.query("SELECT status FROM bpc_pairs WHERE id='pair-1'")).rows[0].status),'rotated');assert.equal(String((await poolA.query("SELECT status FROM bpc_pairs WHERE id='pair-101'")).rows[0].status),'active');const approvedPair=pair(102),approvalRegistration={name:approvedPair.name,scope:approvedPair.scope,mode:approvedPair.mode,secretHash:approvedPair.secretHash,pubJwk:approvedPair.pubJwk};await storeA.setPending('approval-token',approvalRegistration,approvedPair.created);assert.equal(await storeA.approvePending('approval-token',{registration:approvalRegistration,requestedAt:approvedPair.created},approvedPair,100),true);
